...
import { deriveLocationsFromContacts, locationPairKey } from '../model/locations';
import { findDuplicateIds, findLastIndexById } from './duplicateIds';
import {
...
function normalizePhoneHref(phoneDisplay: string) {
...
export function validateDashboardContacts(
  ...
  findDuplicateIds(services.map((service) => service.id)).forEach((id) => {
    const duplicateIndex = findLastIndexById(services, id);
    issues.push({
      level: 'error',
      area: 'contacts',
  ...
  if (hasLocationModel) {
    findDuplicateIds(effectiveLocations.map((location) => location.id)).forEach((id) => {
      const duplicateIndex = findLastIndexById(effectiveLocations, id);
      issues.push({
        level: 'error',
        area: 'contacts',
  ...
  services.forEach((service, index) => {
    validateDashboardContact(issues, service, index, hasLocationModel, locationsById);
  });

  return createValidationResult(issues);
}

function validateDashboardContact(
  issues: DashboardValidationIssue[],
  service: ServiceDirectoryEntry,
  index: number,
  hasLocationModel: boolean,
  locationsById: Map<string, ServiceLocation>,
) {
  if (!service.id.trim()) {
    issues.push({
      level: 'error',
