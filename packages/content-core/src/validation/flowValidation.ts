...
import type { FlowValidationResult, GuidedFlow } from '../model/flowTypes';
import { createValidationResult, type DashboardValidationIssue } from './validationTypes';
import { findDuplicateIds } from './duplicateIds';
...
export interface DashboardFlowValidationOptions {
  /**
   * Flow validator used for the structural pass. Defaults to the canonical core
   * validator; the dashboard facade injects its own seam so existing
   * module-mock based tests keep intercepting the structural pass.
   */
  validateFlowImpl?: (flow: unknown) => FlowValidationResult;
}

export function validateDashboardFlows(
  flows: GuidedFlow[],
  resourceIds: string[],
  { validateFlowImpl = validateFlow }: DashboardFlowValidationOptions = {},
) {
  ...
  flows.forEach((flow) => {
    const validation = validateFlowImpl(flow);
