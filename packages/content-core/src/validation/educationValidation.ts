...
import { DEFAULT_EDUCATION_GROUP_ID } from '../model/groups';
import { isFeaturedImageId } from '../model/featuredImageIds';
import { isImageDataUrl } from './imageDataUrl';
...
export function validateDashboardEducation(resources: EducationResource[], groups: EducationResourceGroup[]) {
  ...
  findDuplicateIds(resources.map((resource) => resource.id)).forEach((id) => {
    const duplicateIndex = findLastIndexById(resources, id);
    issues.push({
      level: 'error',
      area: 'education',
  ...
  resources.forEach((resource) => {
    ...
    validateFeaturedImage(issues, resource);
  ...
  findDuplicateIds(groupIds).forEach((id) => {
    const duplicateIndex = findLastIndexById(groups, id);
    issues.push({
      level: 'error',
      area: 'education',
...
function validateFeaturedImage(issues: DashboardValidationIssue[], resource: EducationResource) {
