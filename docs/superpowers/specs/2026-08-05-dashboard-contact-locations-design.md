...
- Drafts live in `localStorage` (`bemtevi:dev-dashboard:drafts:v1`, schema v5, `DASHBOARD_DRAFT_SCHEMA_VERSION = '5.0.0'`): `contactPatches`, `addedContacts`, `removedContactIds`, `locationPatches`, `addedLocations`, and `removedLocationIds`; `mergeDashboardDrafts` returns the merged content plus normalized `contacts` and `locations`.
- The published payload (`PublishedContentPayload`) has a required in-memory `locations` array; `PAYLOAD_KEYS` still omits it so old rows remain readable. `bundledContent.ts` builds and normalizes the managed list from `canoasServices`; `validateContacts` accepts national contacts only when both denormalized fields are empty.
...
Unlike education groups (which reassign to the default group on removal), there is no natural default location — silently flipping contacts to "Sem local" would be a surprising data mutation. Removal is blocked with the affected contact count and inline guidance to reallocate those contacts first.
...
export interface ServiceLocation {
  id: string; // stable, e.g. 'loc-canoas-rs' / 'location-local-3' for drafts
  city: string;
...
- `deriveLocationsFromContacts(contacts): ServiceLocation[]` — deduped by `(city, state)`, ordered by first appearance, **skipping contacts with empty city**. IDs use a stable slug: `loc-${slug(city)}-${state.toLowerCase()}`, where `slug` lowercases, strips accents, and replaces non-alphanumerics with `-`; colliding slugs receive deterministic hash suffixes. Deterministic across sessions so merge/parse of the same data produces the same IDs.
- `normalizeContactLocations(contacts, locations, { allowDerivation, preserveDenormalizedContactIndexes })` — for each contact: (1) `locationId` resolves → refresh `city`/`state` from the record unless a legacy draft index is being preserved for explicit repair; (2) `locationId` missing but `city`/`state` match a location → attach that id; (3) when `allowDerivation` is true and a legacy contact has both fields set but no `locationId`, derive and append a location; (4) explicit `locationId: null` with stale city/state is not auto-assigned and remains a validation error; empty fields stay national. Explicit location lists are authoritative: unmatched non-empty city/state data is preserved for validation rather than silently adding a location.
- Derived IDs are collision-safe: when distinct city/state pairs share a slug, each receives a deterministic hash suffix. Draft-created IDs use `location-local-N` and never reuse a shipped or removed location ID.
...
- `validateContacts(contacts, locations)` — existing checks plus: `contact.locationId` (when present) must reference a known location id; a referenced contact's denormalized `city`/`state` must exactly match that location; a contact with no location must have both fields empty ("Sem local"). This relaxation is mandatory: without it the feature cannot be published end-to-end (`validatePublicationPayload` at the repository boundary would reject every "Sem local" contact).
- `parsePayload` — if `locations` is omitted, derive it from legacy contacts and allow normalization to attach/derive legacy pairs; if it is present, validate it as authoritative and do not derive orphan pairs. Then run `normalizeContactLocations` and final contact validation. `PAYLOAD_KEYS` (the five strict keys) is unchanged, so old rows still parse.

### `src/app/content/bundledContent.ts`

`getBundledContent()` derives and normalizes `locations` from `canoasServices.services` — guarantees fallback parity (CityFilter options identical to today) and prevents the dashboard baseline from appearing to have phantom contact edits.
...
New pure helper `groupContactsByLocation(services, locations)` (in `contactDrafts.ts` or `locations.ts`): groups in **locations array order** (admin-defined), then a final "Sem local" group for contacts without a resolvable managed location (including invalid/dangling references, so validation issues remain visible).
...
- New location rules using area `'contacts'` (do not extend `DashboardValidationArea` with a new value — locations issues surface in the same tab):
  ...
  - duplicate `(city, state)` pair → summary error `duplicate-location:<canonical-city>|<STATE>`;
...
- **Already-published DB rows** (schema 1.0.0, no `locations`, contacts without `locationId`): parse derives `locations` from contacts and attaches `locationId`s — identical rendering, identical CityFilter options, no forced re-publish. The next admin publish writes the explicit key. Rows that already contain `locations` treat that list as authoritative and fail validation if a contact has an unmatched non-empty city/state pair.
...
- **`mergeDashboardDrafts`** (`dashboardStorage.ts`): additionally merges locations via the existing `mergeRecords` helper (filtered by `removedLocationIds`) and returns `{ ...existing, locations, contacts: normalized }` where contacts+locations pass through `normalizeContactLocations`. Legacy v4 contact patches that still contain city/state are preserved for explicit repair instead of being overwritten by normalization; a new explicit location patch takes precedence.
- **`DashboardRoute.tsx`**:
  - The route normalizes the loaded baseline before comparing it with drafts, so legacy contacts do not create phantom edits. `publishedDraft` and the exported `drafts` object gain `locations: mergedDrafts.locations`; the export `DashboardDraftContent` interface gains `locations` and `removedLocationIds`.
  - New callbacks `onLocationChange/onLocationAdd/onLocationRemove` with origin resolution via `upsertPatchById`: shipped → `locationPatches` upsert; added → `addedLocations` index update; removal is guarded against referenced contacts, filters `addedLocations`, clears `locationPatches`, appends to internal `removedLocationIds`; newly allocated IDs include removed tombstones, including IDs from added locations removed before publication.
  - `contactValidation = useMemo(() => validateDashboardContacts(mergedDrafts.contacts, mergedDrafts.locations), ...)`.
- **`changeSummary.ts`** gains `locations: RecordChangeCount` and includes it in `total`; **`PublishDashboard.tsx`** and **`ExportDashboard.tsx`** show a fifth stat "Locais"; both files' local baseline/draft `PublishedContentPayload` builders include `locations`.
- **`exportBundle.ts`** emits changed location records plus shipped-record `removedLocationIds` (internal tombstones for never-published local locations are filtered out), and uses export schema `4.0.0` so consumers can apply location deletions instead of silently retaining removed locations.
...
1. **`src/domain/services/__tests__/locations.test.ts`** (new): derive dedupes, orders by first appearance, skips empty-city contacts, stable collision-safe IDs; normalize attaches `locationId` from matching legacy city/state; preserves explicit national assignments with stale data for validation; derives missing locations for orphan pairs; refreshes denormalized city/state after a rename; clears city/state for `locationId: null`; `applyLocationSelection` for a location and for "Sem local".
2. **`src/app/content/__tests__/publishedContent.test.ts`**: payload with `locations` parses and normalizes; payload **without** `locations` derives (backward compat); invalid locations rejected (duplicate ID, duplicate pair, bad state, blank city); explicit orphan city/state rejected; contact with dangling `locationId` rejected; "Sem local" contact (empty city/state, no `locationId`) parses; existing missing-keys and size-limit tests unchanged.
3. **`src/dev-dashboard/__tests__/dashboardStorage.test.ts`**: `DASHBOARD_DRAFT_SCHEMA_VERSION` now `'5.0.0'`; v4→v5 migration seeds empty location collections without losing contact drafts; legacy city/state edits remain visible for explicit repair; merge returns `locations` and normalized contacts; `hasDashboardChanges` counts location collections. **All `mergeDashboardDrafts(shipped, ...)` call sites and the typed `emptyDraft` gain `locations` in their shipped literals.**
4. **`src/dev-dashboard/contacts/__tests__/contactsValidation.test.ts`**: dangling `locationId`; city/state mismatch vs. location; leftover city on "Sem local"; duplicate `(city,state)`; duplicate location IDs; blank/invalid managed location fields and paths; zero-locations warning; city allowed empty only when `locationId` is null (update the existing "reports every missing required field" case).
...
7. **`src/dev-dashboard/__tests__/dashboardRoute.test.tsx`** (including the v4 localStorage fixtures) and **`PublishDashboard.test.tsx`**: published draft includes locations; location add/change/remove origin wiring; baseline normalization; "Locais" stat and change counts.
...
9. **`src/dev-dashboard/__tests__/exportBundle.test.ts`**: all `buildExportBundle` calls with `shipped`/`drafts` literals gain `locations`; removed-location tombstones are emitted and the export schema is `4.0.0`.
...
Files: `src/domain/services/types.ts` (new `ServiceLocation`, `locationId`), new `src/domain/services/locations.ts`, `src/app/content/publishedContent.ts` (`PublishedContentPayload.locations` required, `validateLocations`, optional-key parse with legacy-only derivation and authoritative explicit lists, relaxed city/state for "Sem local"), `src/app/content/bundledContent.ts`.
AC: old-payload parse derives locations; explicit orphan pairs, duplicate IDs/pairs, and dangling refs are rejected; collision-safe IDs are deterministic; "Sem local" contacts parse; `validatePublicationPayload` covers locations; new `locations.test.ts` green; all payload construction sites compile.
...
AC: v4 drafts load without loss; merge derives/attaches/refreshes; location IDs never reuse removed tombstones; fixture churn done.
...
AC: location CRUD persisted via drafts with origin resolution; "Locais" stat in both publish and export; publish blocked by location validation errors; removed locations are represented in exports; end-to-end publish round-trips locations into the DB payload and reloads via `parsePayload`.
