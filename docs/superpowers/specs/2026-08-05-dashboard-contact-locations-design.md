...
- **`DashboardRoute.tsx`**:
  ...
  - New callbacks `onLocationChange/onLocationAdd/onLocationRemove` with origin resolution via `upsertPatchById`: shipped → `locationPatches` upsert; added → `addedLocations` index update; removal filters `addedLocations`, clears `locationPatches`, appends to `removedLocationIds`; newly allocated IDs include removed tombstones, including IDs from added locations removed before publication.
...
- **`exportBundle.ts`** emits changed location records plus `removedLocationIds`, and uses export schema `4.0.0` so consumers can apply location deletions instead of silently retaining removed locations.
