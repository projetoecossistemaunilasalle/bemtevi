...
Contract origin: Revision 5; Revision 8 freezes the existing direct-publication policy names used by cutover and otherwise preserves the schema/RPC/publication contract. Public types are in [14-contracts.md](14-contracts.md). All tables below live in `public`. Foreign keys default to `ON DELETE RESTRICT`. Private functions live in unexposed `content_private`. Do not design SQL interfaces from the earlier logical inventory.
...
5. `neon/cutover/20260910004000_content_draft_revoke_direct_publish.sql`: **not automatically applied with additive migrations**. Run only after INTEGRATION-03 verification. Its direct-publication cutover statements are literal and MUST NOT be discovered or renamed by the implementation agent:
   ...
   These are the two write-policy names present at the audited implementation base. Do not drop `"Anyone can read current published content"`; keep public SELECT and admin-only history SELECT. A repository drift that renames/replaces these policies is a change-control event, not permission to guess names.
