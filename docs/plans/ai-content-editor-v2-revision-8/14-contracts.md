...
Contract origin: Revision 5; Revision 8 freezes previously implicit field/unset allowlists without changing the intended V2 operation model. This file defines the wire/domain contract; examples in concern documents are not alternative APIs. Implement these contracts before consumers. No V2 runtime source is delivered by this specification pass.
...
### Exact scope field allowlists

The lists below are normative. Implementers MUST NOT rediscover them from the compatibility `src/dev-dashboard/ai/aiOperations.ts` facade, and a later change to that V1 facade does not change V2. Unknown fields fail the whole batch with `invalid_operations`.

| Scope                | `add.value` allowed keys (with `id` required)                                                                                                      | `update.patch` allowed keys                                                                                                                  | `update.unset` allowed keys                    |
| -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------- |
| `flows`              | `id`, `version`, `locale`, `title`, `type`, `purpose`, `status`, `entry`, `nodes`, `nodeOrder`                                                     | `version`, `locale`, `title`, `type`, `purpose`, `status`, `entry`, `nodes`, `nodeOrder`                                                     | `purpose`, `nodeOrder`                         |
| `educationMaterials` | `id`, `title`, `source`, `description`, `tags`, `audience`, `body`, `embed`, `href`, `group`, `groupOrder`, `review`                               | `title`, `source`, `description`, `tags`, `audience`, `body`, `embed`, `href`, `group`, `groupOrder`, `review`                               | `body`, `embed`, `href`, `group`, `groupOrder` |
| `educationGroups`    | `id`, `title`, `description`, `order`                                                                                                              | `title`, `description`, `order`                                                                                                              | `description`                                  |
| `contacts`           | `id`, `name`, `type`, `badgeTone`, `city`, `state`, `locationId`, `address`, `phoneDisplay`, `phoneHref`, `hours`, `notes`, `lat`, `lng`, `review` | `name`, `type`, `badgeTone`, `city`, `state`, `locationId`, `address`, `phoneDisplay`, `phoneHref`, `hours`, `notes`, `lat`, `lng`, `review` | `locationId`, `hours`, `notes`, `lat`, `lng`   |
| `locations`          | `id`, `city`, `state`                                                                                                                              | `city`, `state`                                                                                                                              | none                                           |

The `educationMaterials` add-key list intentionally excludes top-level `imageUrl`, `imageFileName`, and `featuredImage`; generic add may not set them. Add requires a unique `value.id`, appends to the collection, and preserves explicit order. Updating/deleting a missing ID or adding an existing ID fails the entire batch. Reorder requires a permutation of every ID currently in that scope at that point in the batch; no implicit deletion. `set_default_group_order.value` must be a finite safe integer.

### Protected image fields during generic operations

Only `set_material_image` may set, replace, or clear a retained image slot. Generic operations obey these exact rules:

- generic material add MUST omit top-level `featuredImage`, `imageUrl`, and `imageFileName`; if its optional `body` contains a new `kind: 'image'` block, that block MUST omit `imageUrl`, `imageFileName`, and `alt` until a later `set_material_image` operation in the same or a later batch;
- generic material update/unset cannot name top-level `featuredImage`, `imageUrl`, or `imageFileName`; those keys are intentionally absent from the update/unset allowlists above;
- a generic `body` replacement may add/remove/reorder blocks. For every image block ID present in both old and new body, `imageUrl`, `imageFileName`, and `alt` must be byte/JSON-equal to the old values. A newly introduced image-block ID must omit those three fields. A removed image-block ID is an explicit structural deletion and may disappear with its image;
- unsetting `body` is an explicit structural deletion of all body blocks and is allowed; it does not clear the material's featured or legacy top-level image slots;
- deleting an entire material is an explicit structural deletion and may remove all of its images;
- flow `OrientationVisual` values are read-only in V2 generic operations: a generic flow `add` containing any `visuals` entry is invalid; a generic flow `nodes` patch must preserve the complete `visuals` arrays of every retained node JSON-equally (including visual IDs, `src`, `alt`, order and presence). Adding/removing/reordering/editing flow visuals is `invalid_operations`; deleting the entire flow through explicit `delete` may remove them;
- unchanged existing uploaded/catalog/external/GIF/SVG image values are grandfathered. Restrictions apply only to image changes, not unrelated text edits.

No other image-bearing nested value outside the supported material slots may be introduced or changed by a generic operation. The TypeScript and SQL implementations compare old/new protected slots using these rules; they do not perform a blanket search for `data:` strings. `set_material_image` remains the only operation that can intentionally mutate a retained supported image slot. `reorder` and scalar operations are allowed for all editors, not dashboard-only privileges.
